...
def data_preprocess(data_path_list):
    ...
    # Image.fromarray((train_imgs*255).astype(np.uint8)[0, 0]).save('test0.png')
    # Image.fromarray((train_masks*255).astype(np.uint8)[0, 0]).save('test1.png')
    # Image.fromarray((train_FOVs*255).astype(np.uint8)[0, 0]).save('test2.png')
