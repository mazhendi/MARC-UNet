...
result_list = {"d_up1":'../../experiments/d_up1/result.npy'}
# result_list = {"d_total":'../../experiments/d_total/result.npy'}
...
for name,path in result_list.items():
    ...
    ax.plot(fpr, tpr, label=name + ' (AUC = %0.4f)' % AUC_ROC)
    axins.plot(fpr, tpr, label=name + ' (AUC = %0.4f)' % AUC_ROC)
...
for name,path in result_list.items():
    ...
    ax.plot(recall, precision, label=name+' (AUC = %0.4f)' % AUC_pr)
    axins.plot(recall, precision, label=name+' (AUC = %0.4f)' % AUC_pr)
